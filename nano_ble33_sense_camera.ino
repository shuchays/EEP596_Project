...
/* Includes ---------------------------------------------------------------- */
#include <Basic_Hand_Gesture_inferencing.h>
...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        for (int i = 1; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
          if (result.classification[i].value > result.classification[max_ind].value) {
            max_ind = i;
          }
        }
        if (result.classification[max_ind].value < 0.5) {
          max_ind = 4;
        }
        ...
        switch(max_ind){
          case 0: ei_printf("It's a D! For 'Emotional Damage'\n");
                  nex_y -= 1;
          break;
          case 1: ei_printf("It's a G-ah! Mario!\n");
                  nex_x -= 1;
          break;
          case 2: ei_printf("It's an I? Don't you mean me?\n");
                  nex_y += 1;
          break;
          case 3: ei_printf("It's a Bird! It's a Plane! No, It's an O!\n");
                  nex_x += 1;
          break;
          default: ei_printf("Oh No! Is it paralysis status? You didn't move!\n");
        }
